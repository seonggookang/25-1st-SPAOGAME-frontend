...
class SubjectList extends Component {
  ...
  componentDidMount() {
    fetch(
      `http://192.168.0.133:8000/products/women/outer?offset=0&limit=${LIMIT}`
    )
      .then(res => res.json())
      .then(data => {
        this.setState({
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      fetch(
        `http://192.168.0.133:8000/products/women/outer?offset=${this.state.offset}&limit=${LIMIT}&order_id=${this.state.order_id}`
      )
        .then(res => res.json())
        .then(data => {
          this.setState({
